...
class StudentDashboardController extends Controller {
 public function show(Request $request):JsonResponse {
  ...
  return response()->json(['status'=>true,'data'=>['summary'=>['registered_courses'=>$courseIds->count(),'attendance_percentage'=>$attendance->count()?(int)round($present*100/$attendance->count()):0,'current_cgpa'=>$results->last()['cgpa']??null,'current_semester'=>$currentSemester,'current_semester_number'=>$student->current_semester,'completed_credits'=>$results->last()['completed_credits']??0],'courses'=>$courses,'semester_results'=>$results,'upcoming_classes'=>$schedule,'performance_chart'=>['semesters'=>$results,'courses'=>$byCourse]]]);
 }
 private function emptyData():array{return ['summary'=>['registered_courses'=>0,'attendance_percentage'=>0,'current_cgpa'=>null,'current_semester'=>null,'current_semester_number'=>null,'completed_credits'=>0],'courses'=>[],'semester_results'=>[],'upcoming_classes'=>[],'performance_chart'=>['semesters'=>[],'courses'=>[]]];}
}

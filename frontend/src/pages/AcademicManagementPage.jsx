...
import {
  ...
  saveCourseGrades, saveStudentPerformance, updateCourse,
} from '../services/api';
  ...
  const loadWorkspace = useCallback(async (id) => {
    ...
    try {
      ...
      setPerformance(Object.fromEntries(next.enrollments.map((item) => {
        ...
        return [item.student.id, { semester_gpa: metric.semester_gpa ?? '', cgpa: metric.cgpa ?? '', completed_credits: metric.completed_credits ?? '' }];
      })));
  ...
  return (
    <Layout title="Academic Management" subtitle="Courses, enrollment, attendance, grades, and semester performance.">
      ...
      <div className="academic-tabs mb-4">
        {[['courses', 'Courses'], ['enrollment', 'Enrollment'], ['attendance', 'Attendance'], ['results', 'Grades & CGPA']].map(([key, label]) => <button key={key} type="button" className={tab === key ? 'is-active' : ''} onClick={() => setTab(key)}>{label}</button>)}
      </div>
      ...
      {tab !== 'courses' && <>
        ...
        {!workspace ? <EmptyState title="Select a course" message="Choose a course to enter academic data." /> : tab === 'enrollment' ? <div className="row g-4">
          ...
          <div className="col-xl-7"><section className="faculty-panel"><h4>Semester GPA & CGPA</h4><div className="academic-performance-list">{workspace.enrollments.map((item) => { const entry = performance[item.student.id] || {}; return <article key={item.student.id}><div><strong>{item.student.name}</strong><small>{item.student.student_number}</small></div>{[['semester_gpa', 'Semester GPA'], ['cgpa', 'CGPA'], ['completed_credits', 'Credits']].map(([key, label]) => <label key={key}><span>{label}</span><input type="number" step={key === 'completed_credits' ? 1 : .01} className="form-control" value={entry[key] ?? ''} onChange={(e) => setPerformance({ ...performance, [item.student.id]: { ...entry, [key]: e.target.value } })} /></label>)}<button type="button" className="btn btn-sm btn-outline-primary" onClick={() => action('performance', () => saveStudentPerformance(courseId, item.student.id, { semester: term.semester, year: Number(term.year), semester_gpa: Number(entry.semester_gpa), cgpa: Number(entry.cgpa), completed_credits: Number(entry.completed_credits) }), 'Performance updated.')}>Update</button></article>; })}</div></section></div>
        </div>}
      </>}
    </Layout>
  );

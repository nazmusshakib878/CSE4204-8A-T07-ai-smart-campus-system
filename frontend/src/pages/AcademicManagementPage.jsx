      ...
      {tab === 'courses' && <>
        {isAdmin && <form className="faculty-panel mb-4" onSubmit={submitCourse}>
          ...
          <div className="row g-3">
            ...
            <div className="col-md-5"><label className="form-label">Assigned faculty (Faculty ID)</label><select className="form-select" value={courseForm.faculty_id} onChange={(e) => setCourseForm({ ...courseForm, faculty_id: e.target.value })}><option value="">Unassigned</option>{data.faculty.map((member) => <option key={member.id} value={member.id}>{member.faculty_id} · {member.name}</option>)}</select></div>

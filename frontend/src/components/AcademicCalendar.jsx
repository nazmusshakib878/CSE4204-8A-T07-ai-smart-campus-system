...
export default function AcademicCalendar({ user, data, reload, setFeedback }) {
  ...
  const calendarEvents = useMemo(() => [
    ...
    ...cells.filter((date) => date.getDay() === 5).map((date) => ({ id: `weekly-holiday-${iso(date)}`, title: 'Weekly Holiday', starts_on: iso(date), ends_on: iso(date), event_type: 'holiday', audience: 'all', virtual: true })),
  ], [cells, data]);

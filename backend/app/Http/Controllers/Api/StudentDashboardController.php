...
class StudentDashboardController extends Controller {
 public function show(Request $request):JsonResponse {
  ...
  $gradebook=$student->academicRecords()->withoutDemo()->with('course:id,course_code,title,credit_hours')->orderByDesc('semester_number')->orderByDesc('year')->orderByDesc('id')->get()->map(fn($record)=>['id'=>$record->id,'course_id'=>$record->course_id,'course_code'=>$record->course_code_snapshot?:$record->course?->course_code,'course_title'=>$record->course_title_snapshot?:$record->course?->title,'credits'=>(float)($record->credit_hours_snapshot??$record->course?->credit_hours??0),'grade'=>$record->grade,'grade_point'=>$record->grade_point,'semester'=>$record->semester,'semester_number'=>$record->semester_number,'year'=>$record->year])->values();  return response()->json(['status'=>true,'data'=>['summary'=>['registered_courses'=>$courseIds->count(),'attendance_percentage'=>$attendance->count()?(int)round($present*100/$attendance->count()):0,'current_cgpa'=>$results->last()['cgpa']??null,'current_semester'=>$currentSemester,'current_semester_number'=>$student->current_semester,'section'=>$student->section,'completed_credits'=>$results->last()['completed_credits']??0],'courses'=>$courses,'attendance_records'=>$attendanceRecords,'gradebook'=>$gradebook,'semester_results'=>$results,'upcoming_classes'=>$schedule,'performance_chart'=>['semesters'=>$results,'courses'=>$byCourse]]]);
 }

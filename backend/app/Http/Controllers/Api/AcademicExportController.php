...
class AcademicExportController extends Controller
{
    public function transcript(Request $request, ?Student $student = null)
    {
        ...
        $student->load(['user', 'academicRecords' => fn ($query) => $query->withoutDemo()->with('course'), 'performanceMetrics']);
        $rows = $student->academicRecords->sortBy(fn ($r) => sprintf('%04d-%s-%s', $r->year, $r->semester, ($r->course_code_snapshot ?: $r->course?->course_code)))
            ->map(fn ($r) => '<tr><td>'.e(($r->course_code_snapshot ?: $r->course?->course_code)).'</td><td>'.e($r->course_title_snapshot ?: $r->course?->title).'</td><td>'.e($r->semester.' '.$r->year).'</td><td>'.e($r->credit_hours_snapshot ?? $r->course?->credit_hours).'</td><td>'.e($r->grade).'</td></tr>')->implode('');
        $metric = $student->performanceMetrics->sortByDesc('created_at')->first();
        $html = '<!doctype html><html><head><meta charset="utf-8"><title>Academic Transcript</title><style>body{font-family:Arial;margin:40px;color:#172033}h1{color:#174ea6;margin-bottom:4px}table{width:100%;border-collapse:collapse;margin-top:24px}th,td{border:1px solid #ccd5e4;padding:10px;text-align:left}th{background:#eef4ff}.summary{margin-top:24px;padding:16px;background:#f4f7fb}.print{float:right;padding:10px 18px}@media print{.print{display:none}}</style></head><body><button class="print" onclick="window.print()">Print / Save PDF</button><h1>Northern University of Business and Technology, Khulna</h1><h2>Academic Transcript</h2><p><strong>Student:</strong> '.e($student->user?->name).'<br><strong>ID:</strong> '.e($student->student_number).'<br><strong>Department:</strong> '.e($student->department).'<br><strong>Section:</strong> '.e($student->section ?? 'N/A').'</p><table><thead><tr><th>Code</th><th>Course</th><th>Term</th><th>Credits</th><th>Grade</th></tr></thead><tbody>'.$rows.'</tbody></table><div class="summary"><strong>CGPA:</strong> '.e($metric?->cgpa ?? 'N/A').' &nbsp; <strong>Completed credits:</strong> '.e($metric?->completed_credits ?? 0).'</div><script>window.onload=()=>window.print()</script></body></html>';

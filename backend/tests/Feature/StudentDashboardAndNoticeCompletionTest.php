...
class StudentDashboardAndNoticeCompletionTest extends TestCase {
 ...
 public function test_student_dashboard_returns_only_personal_academic_data():void {
  ...
  AcademicRecord::create(['student_id'=>$student->id,'course_id'=>$course->id,'grade'=>'A','semester'=>'Spring','year'=>2026]);
  ...
  $this->getJson('/api/student/dashboard')->assertOk()->assertJsonPath('data.summary.registered_courses',1)->assertJsonPath('data.summary.attendance_percentage',100)->assertJsonPath('data.summary.current_cgpa',3.4)->assertJsonPath('data.summary.current_semester','Spring 2026')->assertJsonPath('data.summary.current_semester_number',8)->assertJsonPath('data.courses.0.code','CSE-101')->assertJsonPath('data.attendance_records.0.status','present')->assertJsonPath('data.gradebook.0.grade','A')->assertJsonCount(1,'data.upcoming_classes');
 }
